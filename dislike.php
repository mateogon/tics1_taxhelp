<?php
    ...
    if (in_array($session_id, (array)$post['dislikes-users'])){//already disliked
        ...
        $updateResult = $collection->updateOne(
            ...
            [ 
            '$inc' => ['dislikes' => 1] ,
            '$addToSet' => ["dislikes-users" => $session_id]
            ]
            );
        if (in_array($session_id, (array)$post['likes-users'])){//checks if had liked previously
            $updateResult = $collection->updateOne(
                [ '_id' => new MongoDB\BSON\ObjectId($_GET['id']) ],
                [ '$inc' => ['likes' => -1],
